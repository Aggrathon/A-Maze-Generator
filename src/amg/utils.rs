...
pub(super) fn get_num_diff_neighbours(arr: &[i32], i: usize, width: usize) -> DiffNeigh {
    ...
    if i == 0 {
        return DiffNeigh::None;
    }
    if i == 1 {
        return DiffNeigh::One;
    }
    for j in 1..i {
        if dir[j] != dir[j - 1] {
            return DiffNeigh::MultDiff;
        }
    }
    DiffNeigh::MultSame
}
...
mod tests {
    ...
    fn test_num() {
        ...
        assert_eq!(
            DiffNeigh::MultSame,
            get_num_diff_neighbours(&[1, 0, 1], 1, 3)
        );
        assert_eq!(
            DiffNeigh::MultDiff,
            get_num_diff_neighbours(&[1, 0, 2], 1, 3)
        );
        assert_eq!(DiffNeigh::Error, get_num_diff_neighbours(&[1, 0, 2], 5, 3));
        assert_eq!(
            DiffNeigh::MultDiff,
            get_num_diff_neighbours(&[0, 3, 0, 1, 0, 2, 0, 4, 0,], 4, 3)
        );
    }
    ...
    fn test_low() {
        assert_eq!(
            get_lowest_neighbour(&[0, 1, 0, 0, 0, 0, 0, 0, 0], 4, 3, 99),
            1
        );
        assert_eq!(
            get_lowest_neighbour(&[0, 0, 0, 1, 0, 0, 0, 0, 0], 4, 3, 99),
            1
        );
        assert_eq!(
            get_lowest_neighbour(&[0, 0, 0, 0, 0, 1, 0, 0, 0], 4, 3, 99),
            1
        );
        assert_eq!(
            get_lowest_neighbour(&[0, 0, 0, 0, 0, 0, 0, 1, 0], 4, 3, 99),
            1
        );
        assert_eq!(
            get_lowest_neighbour(&[0, 1, 0, 0, 0, 2, 0, 0, 0], 4, 3, 99),
            1
        );
        assert_eq!(
            get_lowest_neighbour(&[0, 1, 0, 1, 0, 1, 0, 1, 0], 4, 3, 99),
            1
        );
    }
}
