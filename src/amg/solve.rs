...
mod tests {
    ...
    fn test_recursive() {
        ...
        for p in path.iter().skip(1) {
            ...
            assert!(
                (prev.0 as i32 - next.0 as i32).abs() + (prev.1 as i32 - next.1 as i32).abs() <= 1
            );
            prev = next;
        }
        assert_eq!(path[0], 7);
    ...
    fn test_image_path() {
        ...
        add_path_to_maze_image(
            &maze,
            &[9, 10, 16, 17, 23],
            &mut img,
            Rgb([255u8, 128u8, 128u8]),
            1,
        );
